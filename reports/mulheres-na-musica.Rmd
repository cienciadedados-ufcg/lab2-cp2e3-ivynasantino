---
title: "Análise dos movimentos musicais a partir de vozes femininas"
...
Cada movimento teve sua particularidade, empiricamente temos conhecimentos de que a jovem guarda foi marcado por músicas animadas, que agitaram a juventude de muitos na década de 60 e 70, a Bossa nova pela música de boêmios e clubes, e o Tropicalismo marcado pela música de protesto aos anos de chumbo no Brasil. Então a partir disso, podemos observar alguns padrões diante fatores como a dançabilidade, discurso, acústica e instrumentabilidade em cada canção. 

Diante dos dados, podemos ver que músicas da jovem guarda não são muito populares, mas possuem altos índices de dançabilidade, mostrando um certo padrão entre as músicas.
...
Embora as artistas pertençam aos dois movimentos musicais, suas músicas são mais populares com a Bossa, mesmo tendo Lindonéia e Deus lhe pague com altos índices popularidade não conseguem superar as suas faixas da Bossa nova.


### Ao longo do tempo, as músicas de cada movimento ficam mais populares ou o movimento vai enfraquecendo?

```{r, echo=FALSE, warning=FALSE, message=FALSE}
#2.2 se der p deixar o plot com animaçao depois
cancoes %>% 
    plot_ly(y = ~track_popularity,
            x = ~year,
            color = ~style,
...
Com o passar do tempo, o movimento da jovem guarda vai perdendo a popularidade, pois na década de 60 estava em alta, mas no início da década de 70 vai enfraquecendo. Já o Tropicalismo, vai ganhando popularidade ao decorrer dos anos e podemos notar que as artistas que estamos analisando produziram muito em menos de 10 anos para esse movimento, pois há uma grande concentração das músicas entre a década de 60 e 70, devido a implantação da ditadura militar. Já a Bossa, também foi perdendo força ao longo dos anos, no entanto permaneceu por mais tempo desde a década de 50 até 80.

Diante dessas análises acima, podemos concluir alguns padrões nas músicas de cada movimento, descobrir fatos curiosos com relação ao humor que cada canção desperta no público. Lembrando que esses dados são fornecidos pela API do Spotify, dessa forma alguns desses dados podem estar impressisos.
